export const listeningData = [
  {
    title: "Audio 1 : kiku1",
    audio: "audio/Y_[10-01]_kiku1.mp3",
    script: [
      "A：すみません。",
      "B：はい。",
      "A：日本語教室【きょうしつ】に参加【さんか】したいんですが……。いつありますか?",
      "B：日本語教室ですね? 水曜日の教室と、土曜日の教室があります。",
      "A：何時からですか?",
      "B：水曜日は夜【よる】の7時から8時半、土曜日は朝の10時から11時半です。",
      "A：じゃあ、夜のクラスがいいです。"
    ],
    image: [
      "images/listen/10-01.png"
    ],
    translation: [
      "A: Xin lỗi.",
      "B: Vâng.",
      "A: Tôi muốn tham gia lớp học tiếng Nhật… Lớp học được tổ chức khi nào?",
      "B: Lớp tiếng Nhật phải không? Có lớp vào thứ Tư và lớp vào thứ Bảy.",
      "A: Mấy giờ bắt đầu?",
      "B: Thứ Tư từ 7 giờ tối đến 8 giờ rưỡi, thứ Bảy từ 10 giờ sáng đến 11 giờ rưỡi.",
      "A: Vậy thì tôi chọn lớp buổi tối."
    ]

  },
  {
    title: "Audio 2 : kiku2",
    audio: "audio/Y_[10-02]_kiku2.mp3",
    script: [
      "A：すみません。合気道【あいきどう】をやってみたいんですが……。",
      "B：あ、合気道教室ですね。",
      "A：まだ、空【あ】いていますか?",
      "B：だいじょうぶですよ。",
      "A：えーと、合気道の服は必要【ひつよう】ですか?",
      "B：服ですか? 最初【さいしょ】はTシャツでもだいじょうぶですよ。あとは先生に聞いてください。"
    ],
    image: [
      "images/listen/10-01.png"
    ],
    translation: [
      "A: Xin lỗi. Tôi muốn thử tập Aikido…",
      "B: À, lớp Aikido phải không?",
      "A: Còn chỗ trống không?",
      "B: Không sao đâu, vẫn còn.",
      "A: Ừm, có cần mặc võ phục không?",
      "B: Võ phục à? Lúc đầu mặc áo thun cũng được. Sau đó hãy hỏi thầy giáo."
    ]

  },
  {
    title: "Audio 3 : kiku3",
    audio: "audio/Y_[10-03]_kiku3.mp3",
    script: [
      "A：すみません。この料理教室に参加したいんですが……。",
      "B：料理教室ですね。少々お待ちください。",
      "A：あ、金曜日のクラスは、もういっぱいです。木曜日のクラスでもいいですか？",
      "B：はい、だいじょうぶです。",
      "A：あのう、いくらですか？",
      "B：１回800円払【はら】ってください。"
    ],
    image: [
      "images/listen/10-01.png"
    ],
    translation: [
      "A: Xin lỗi. Tôi muốn tham gia lớp học nấu ăn…",
      "B: Lớp nấu ăn phải không? Xin vui lòng chờ một chút.",
      "A: À, lớp thứ Sáu đã kín chỗ rồi. Lớp thứ Năm cũng được chứ?",
      "B: Vâng, không sao đâu.",
      "A: À, học phí là bao nhiêu?",
      "B: Mỗi buổi hãy trả 800 yên."
    ]

  },
  {
    title: "Audio 4 : kiku4",
    audio: "audio/Y_[10-04]_kiku4.mp3",
    script: [
      "A：すみません。私は書道【しょどう】に興味【きょうみ】があります。",
      "A：書道【しょどう】教室に参加してみたいんですが……。",
      "B：体験【たいけん】教室ですか？それとも毎週のコースですか？",
      "A：体験【たいけん】教室です。いつありますか？",
      "B：次は5月14日の日曜日です。",
      "A：そうですか。道具【どうぐ】は必要ですか？",
      "B：全部【ぜんぶ】こちらで用意【ようい】しますから、だいじょうぶですよ。"
    ],
    image: [
      "images/listen/10-01.png"
    ],
    translation: [
      "A: Xin lỗi. Tôi có hứng thú với thư pháp.",
      "A: Tôi muốn thử tham gia lớp học thư pháp…",
      "B: Là lớp trải nghiệm à? Hay là khóa học hàng tuần?",
      "A: Là lớp trải nghiệm. Khi nào được tổ chức?",
      "B: Buổi tiếp theo là Chủ Nhật, ngày 14 tháng 5.",
      "A: Vậy à. Có cần chuẩn bị dụng cụ không?",
      "B: Tất cả sẽ được chuẩn bị ở đây, nên không sao đâu."
    ]
  },
  {
    title: "Audio 5 : katachi",
    audio: "audio/Y_[10-05]_katachi.mp3",
    script: [
      "日本語教室に参加＿＿＿＿。",
      "合気道をやって＿＿＿＿。",
      "書道教室に参加して＿＿＿＿。",
    ],
    translation: [
      "1. Tôi muốn tham gia lớp học tiếng Nhật (… thì thế nào nhỉ?). → 参加したいんですが",
      "2. Tôi muốn tập thử Aikido (… thì thế nào nhỉ?). → やってみたいんですが",
      "3. Tôi muốn tham gia lớp thư pháp (… thì thế nào nhỉ?). → 参加してみたいんですが"
    ]
  },
  {
    title: "Audio 6 : kaiwa",
    audio: "audio/Y_[10-06]_kaiwa.mp3",
    script: [
      "スニル：ユーリヤさん、ちょっといいですか？",
      "ユーリヤ：はい。",
      "スニル：今、日本語の教室【きょうしつ】に行っていますか？",
      "ユーリヤ：はい。本町【もとまち】公民館の日本語教室で勉強しています。",
      "スニル：私も、もっと日本語を勉強したいんですが、クラスはどうですか？",
      "ユーリヤ：楽しいですよ。",
      "スニル：週【しゅう】に何回ありますか？",
      "ユーリヤ：私のクラスは週に１回です。火曜日の夜【よる】です。",
      "スニル：何時からですか？",
      "ユーリヤ：７時から８時半です。土曜日の午前のクラスもありますよ。",
      "スニル：いくらですか？",
      "ユーリヤ：ただです。でも、教科書は自分で買います。",
      "スニル：そうですか。先生は、どんな先生ですか？",
      "ユーリヤ：いろいろな先生がいます。みんな親切ですよ。",
      "スニル：へー。",
      "ユーリヤ：今度、教室のチラシを持って来ましょうか？",
      "スニル：ありがとうございます。じゃあ、お願いします。"
    ],

    translation: [
      "スニル: Tôi có thể làm phiền một chút không?",
      "ユーリヤ: Vâng.",
      "スニル: Bây giờ bạn có đang đi học lớp tiếng Nhật không?",
      "ユーリヤ: Vâng. Tôi đang học ở lớp tiếng Nhật của nhà văn hóa Otomachi.",
      "スニル: Tôi cũng muốn học tiếng Nhật nhiều hơn, lớp học thế nào?",
      "ユーリヤ: Vui lắm.",
      "スニル: Một tuần có mấy buổi?",
      "ユーリヤ: Lớp của tôi mỗi tuần 1 buổi. Tối thứ Ba.",
      "スニル: Mấy giờ bắt đầu?",
      "ユーリヤ: Từ 7 giờ đến 8 rưỡi. Cũng có lớp buổi sáng thứ Bảy nữa.",
      "スニル: Học phí bao nhiêu?",
      "ユーリヤ: Miễn phí. Nhưng phải tự mua giáo trình.",
      "スニル: Vậy à. Giáo viên là người như thế nào?",
      "ユーリヤ: Có nhiều giáo viên khác nhau. Mọi người đều rất tốt bụng.",
      "スニル: À vậy à.",
      "ユーリヤ: Lần tới tôi mang tờ rơi của lớp đến nhé?",
      "スニル: Cảm ơn bạn. Vậy thì nhờ bạn nhé."
    ]
  },
  {
    title: "Audio 7 : katachi",
    audio: "audio/Y_[10-07]_katachi.mp3",
    script: [
      "私も、もっと日本語を勉強したい____________、クラスはどうですか？",
      "A：週に____________ありますか？",
      "B：私のクラスは週に____________です。",
      "A：今度、教室のチラシを持って来____________？",
      "B：ありがとうございます。じゃあ、お願いします。"
    ],
    image: [
      "",
    ],
    translation: [
      "1. Tôi cũng muốn học tiếng Nhật nhiều hơn, nên muốn hỏi lớp học thế nào. → んですが",
      "2. A: Một tuần có mấy buổi? → 何回",
      "3. B: Lớp của tôi mỗi tuần 1 buổi. → １回",
      "4. A: Lần tới, tôi mang tờ rơi của lớp đến cho bạn nhé? → 来ましょうか",
      "5. B: Cảm ơn bạn. Vậy thì nhờ bạn nhé."
    ]
  },
  {
    title: "Audio 8 : hanasu",
    audio: "audio/Y_[10-08]_hanasu.mp3",
    script: [
      "1.私も、もっと日本語を勉強したいんですが、クラスはどうですか？",
      "2.楽しいですよ。",
      "3.週に何回ありますか？",
      "4.週に１回です。火曜日です。",
      "5.何時からですか？",
      "6.夜７時から８時半です。",
      "7.いくらですか？",
      "8.ただです。でも、教科書は自分で買います。",
      "9.先生は、どんな先生ですか？",
      "10.親切です。",
      "11.そうですか。",
      "12.今度、教室のチラシを持って来ましょうか",
      "11.ありがとうございます。じゃ、お願いします。",

    ],

    image: [
      "images/listen/10-08.png"
    ],
    translation: [
      "1. Tôi cũng muốn học thêm tiếng Nhật, lớp học thế nào?",
      "2. Vui lắm.",
      "3. Một tuần có mấy buổi?",
      "4. Một tuần một buổi. Vào thứ Ba.",
      "5. Bắt đầu từ mấy giờ?",
      "6. Từ 7 giờ tối đến 8 giờ rưỡi.",
      "7. Học phí bao nhiêu?",
      "8. Miễn phí. Nhưng sách giáo khoa phải tự mua.",
      "9. Thầy/cô giáo là người như thế nào?",
      "10. Rất thân thiện.",
      "11. Vậy à.",
      "12. Lần tới tôi sẽ mang tờ rơi của lớp học đến nhé",
      "13. Xin cám ơn. Vậy thì nhờ bạn nhé.",
    ]

  },
  {
    title: "Audio 9 : kaiwa",
    audio: "audio/Y_[10-09]_kaiwa.mp3",
    script: [
    ],
    translation: [
    ]
  },
  {
    title: "Audio 10 : katachi",
    audio: "audio/Y_[10-10]_katachi.mp3",
    script: [
    ],
    image: [
      "",
    ],
    translation: [
    ]
  },
  {
    title: "Audio 11 : hanasu",
    audio: "audio/Y_[10-11]_hanasu.mp3",
    script: [
    ],

    translation: [
    ]
  },
  
];